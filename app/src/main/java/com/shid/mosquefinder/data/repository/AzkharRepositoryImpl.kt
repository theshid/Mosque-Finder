...
class AzkharRepositoryImpl @Inject constructor(
    val resources: Resources,
    private val deeplApiInterface: DeeplApiInterface
) :
    AzkharRepository {

    //val service = Common.deeplApiService
    private val TARGET_LG = "FR"
    ...
    override suspend fun setTranslation(input: String): Flow<DeepL> = flow {
        val response = deeplApiInterface.getTranslation(
            resources.getString(R.string.deep_key),
            input,
            TARGET_LG
        )
